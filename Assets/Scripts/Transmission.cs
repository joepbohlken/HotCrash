...
public class Transmission : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        while ((skipNeutral) && gears[Mathf.Clamp(currentGear + upGearOffset, 0, gears.Length - 1)].ratio == 0 && currentGear + upGearOffset != 0 && currentGear + upGearOffset < gears.Length - 1)
        {
            upGearOffset++;
        }

        while ((skipNeutral) && gears[Mathf.Clamp(currentGear - downGearOffset, 0, gears.Length - 1)].ratio == 0 && currentGear - downGearOffset != 0 && currentGear - downGearOffset > 0)
        {
            downGearOffset++;
        }
        ...
        {
            newDrive.rpm = (skidSteerDrive ? Mathf.Abs(targetDrive.rpm) * Mathf.Sign(carController.accelInput - (carController.brakeInput * (1 - carController.burnout))) : targetDrive.rpm) / curGearRatio;
        ...
        if (shiftTime == 0 && carController.groundedWheels > 0)
        {
            if (!skidSteerDrive && carController.burnout == 0)
            {
                if (Mathf.Abs(carController.localVelocity.z) > 1 || carController.accelInput > 0 || (carController.brakeInput > 0))
                {
                    if (currentGear < gears.Length - 1
                        && (upperGear.minRPM + upshiftDifference * (curGearRatio < 0 ? Mathf.Min(1, shiftThreshold) : shiftThreshold) - actualFeedbackRPM <= 0 || (curGearRatio <= 0 && upperGear.ratio > 0 && (!carController.reversing || (carController.accelInput > 0 && carController.localVelocity.z > curGearRatio * 10))))
                        && !(carController.brakeInput > 0 && upperGear.ratio >= 0)
                        && !(carController.localVelocity.z < 0 && carController.accelInput == 0))
                    {
                        Debug.Log("shift up");
                        Shift(1);
                    }
                    else if (currentGear > 0
                        && (actualFeedbackRPM - (lowerGear.maxRPM - downshiftDifference * shiftThreshold) <= 0 || (curGearRatio >= 0 && lowerGear.ratio < 0 && (carController.reversing || ((carController.accelInput < 0 || (carController.brakeInput > 0)) && carController.localVelocity.z < curGearRatio * 10))))
                        && !(carController.accelInput > 0 && lowerGear.ratio <= 0)
                        && (lowerGear.ratio > 0 || carController.localVelocity.z < 1))
                    {
                        Shift(-1);
                    }
                }
    ...
    private void SetOutputDrives(float ratio)
    {
    ...
    public void Shift(int dir)
    {
        if (health > 0)
        {
            ...
            while ((skipNeutral) && gears[Mathf.Clamp(currentGear, 0, gears.Length - 1)].ratio == 0 && currentGear != 0 && currentGear != gears.Length - 1)
            {
                currentGear += dir;
            }
    ...
    public void CalculateRpmRanges()
    {
        ...
        {
            Engine engine = transform.GetComponentInParent<CarController>().GetComponentInChildren<Engine>();
